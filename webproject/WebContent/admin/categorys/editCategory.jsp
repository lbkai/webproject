...
<body>
<form id="myform" action="doEditCategory.jsp" method="post">
	<table>
	    <tr>
			<td>目录名称</td><td><input type="text" name="categoryID" value="${ca.categoryID  }" readonly="readonly" /></td>
			
		</tr>
		<tr>
			<td>目录名称</td><td><input id="category_name" type="text" name="category_name" value="${ca.category_name }" onclick="checkName()" onchange="checkName()"  onblur="checkName()" /></td>
			<td><label id="nameresult" ></label></td>
		</tr>
		<tr>
			<td>备注</td><td><input id="category_desc" type="text" name="category_desc" value="${ca.category_desc }" onclick="checkDesc()" onchange="checkDesc()"  onblur="checkDesc()" /></td>
			<td><label id="descresult" ></label></td>
		</tr>
		<tr>
			<td colspan="2">
			<!-- onclick="return submitForm()" submitForm()方法为真实提交表单，否则不提交 -->
			<input type="button" value="确定修改" onclick="checkAll1()" />
